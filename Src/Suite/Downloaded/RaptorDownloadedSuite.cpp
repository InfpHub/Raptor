...
#include "RaptorDownloadedSuite.h"

Q_GLOBAL_STATIC(RaptorDownloadedSuite, _DownloadedSuite)

RaptorDownloadedSuite* RaptorDownloadedSuite::invokeSingletonGet()
{
    return _DownloadedSuite();
}

void RaptorDownloadedSuite::onItemsLoading() const
{
...
void RaptorDownloadedSuite::onItemsClearing() const
{
    auto qSQLQuery = RaptorPersistenceSuite::invokeQueryGenerate();
    const auto qSQL = R"(
        DELETE FROM Downloaded WHERE UserId = :UserId
    )";
    qSQLQuery.prepare(qSQL);
    qSQLQuery.bindValue(":UserId", RaptorStoreSuite::invokeUserGet()._LeafId);
    qSQLQuery.exec();
...
void RaptorDownloadedSuite::onItemsDeleting(const QVariant& qVariant) const
{
    auto qSQLQuery = RaptorPersistenceSuite::invokeQueryGenerate();
    ...
    const auto qSQL = R"(
        DELETE FROM Downloaded WHERE LeafId = :LeafId AND UserId = :UserId
    )";
...
void RaptorDownloadedSuite::onItemCompleted(const QVariant& qVariant) const
{
