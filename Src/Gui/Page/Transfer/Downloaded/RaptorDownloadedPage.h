...
#ifndef RAPTORDOWNLOADEDPAGE_H
#define RAPTORDOWNLOADEDPAGE_H
...
#include "../../../../Model/Page/Transfer/Downloaded/RaptorDownloadedViewModel.h"
...
namespace Ui
{
    class RaptorDownloadedPage;
}

QT_END_NAMESPACE

class RaptorDownloadedPage Q_DECL_FINAL : public QWidget
{
    Q_OBJECT

public:
    explicit RaptorDownloadedPage(QWidget* qParent = Q_NULLPTR);

    ~RaptorDownloadedPage() Q_DECL_OVERRIDE;
...
private:
    Ui::RaptorDownloadedPage* _Ui = Q_NULLPTR;
    RaptorDownloadedViewModel* _ItemViewModel = Q_NULLPTR;
    RaptorTableViewHeader* _ItemViewHeader = Q_NULLPTR;
    RaptorTableViewDelegate* _ItemViewDelegate = Q_NULLPTR;
};

#endif // RAPTORDOWNLOADEDPAGE_H